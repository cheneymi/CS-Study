...
// 回溯法遍历
void search(int data[],int n,int i,int prime[]) {
    if (i==n){    // 递归边界，n个数都确定了位置
        if (prime[data[n]+data[1]]) {
    ...
    for (int j=i;j<=n;j++){  // 枚举尚未确定的数字
        if (prime[data[j]+data[i-1]]){ // 剪枝
            data[i]=data[j];    // 交换i,j位的数字
            data[j]=tmp;
            search(data,n,i+1,prime);   // 递归搜索下一个数
...
int main(){
    ...
    int data[n+1], prime[2*n];
    for (int j=1;j<=n;j++) {    // 初始化排列
        data[j]=j;
    }
    for (int j=1;j<=2*n-1;j++) {    // 预计算
        prime[j]=isPrime(j);
    }

    search(data,n,2,prime);   // 为什么从2开始？因为循环对称性，第1位固定为1}
    return 0;
}
