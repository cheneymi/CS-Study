...
// Su,Ruizhe ^
int countInversePair2(int data[],int head,int tail) {
    if (head==tail) { // 递归边界
        return 0;
    }
    int mid=(head+tail)/2;
    int result=countInversePair2(data,head,mid)
    +countInversePair2(data,mid+1,tail); // 左右子列结果
    // 归并排序同时计算交叉的逆序对数
...
//    int ans=countInversePair(a,0,n-1);  // 计算逆序对数
     int ans=countInversePair2(a,0,n-1);
    cout<<ans<<endl;    // 输出
    return 0;
}
